...
VALUES (sqlc.arg(id)::uuid, sqlc.arg(code), sqlc.arg(password), sqlc.arg(type),
...
-- name: GetClassByIdAndStatus :one
SELECT * FROM classes WHERE id = sqlc.arg(id)::uuid AND status = sqlc.arg(status);
